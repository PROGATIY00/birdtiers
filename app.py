"""
MAGMATIERS INTEGRATED SYSTEM - VERSION 4.7.1
--------------------------------------------
- FIXED: PyMongo NotImplementedError (Truth value testing).
- AUTHENTICATION: Integrated Discord OAuth2 Login.
- FEATURES: Decline System, High Results, Profiles, Maintenance, Ban/Retire.
- PERFORMANCE: 15-second live refresh.
"""
...
# OAuth2 Config (Must be set in your Environment Variables)
...
def get_rank_name(tier_list):
    ...
    score = calculate_player_score(tier_list)
...
def is_maintenance_active():
    # FIXED: Explicitly checking against None to satisfy PyMongo truth-testing rules
...
def home():
    ...
    if maint['active']:
        return f"<html><head>{STYLE}</head><body style='display:flex; justify-content:center; align-items:center; height:100vh;'><h1>🛠️ {maint['reason']}</h1></body></html>"
    ...
    if search_q:
        ...
        if match: spotlight = match

    processed = []
    for u, data in users.items():
        if not data["tiers"] and not spotlight: continue
        best = max(data["tiers"], key=get_tier_value) if data["tiers"] else "N/A"
        entry = {"u": u, "t": data["kits"].get(mode_q, best), "rn": get_rank_name(data["tiers"]), "reg": data['reg'], "score": calculate_player_score(data["tiers"])}
    ...
    <body>
        <div class="header">
            ...
            <div style="display:flex; gap:10px;">
                ...
                <a href="/moderation" class="nav-btn">MOD PANEL</a>
            </div>
        </div>
        {% if spotlight %}
        <div class="modal-bg" onclick="window.location.href='/'"><div class="modal" onclick="event.stopPropagation()">
            <img src="https://minotar.net/helm/{{ spotlight.u }}/64.png" style="margin-bottom:10px;">
            <h2>{{ spotlight.u }}</h2><span class="badge">{{ spotlight.rn }}</span>
            <div style="margin-top:20px;">
                {% for s in spotlight.all %}<div class="{% if s.retired %}retired{% endif %}">{{ s.gamemode }}: <b>{{ s.tier }}</b></div>{% endfor %}
            </div>
        </div></div>
        {% endif %}
        <div class="container">
            {% if not mode_q and high_p %}
            <div class="high-results"><b>🔥 HIGH RESULTS</b>
                {% for h in high_p[:3] %}<div class="player-row" style="border-color: gold;"><div>⭐</div><img src="https://minotar.net/helm/{{h.u}}/32.png"><div>{{h.u}}</div><div>{{h.reg}}</div><div style="color:var(--accent)">{{h.t}}</div></div>{% endfor %}
            </div>
            {% endif %}
            {% for p in players %}
            <a href="/?search={{ p.u }}" class="player-row">
                <div style="font-weight:800;">#{{ loop.index }}</div>
    ...
    <div class="container">
        ...
        <div style="background:var(--card); padding:20px; border-radius:12px; margin-bottom:10px; border:1px solid var(--border);">
            ...
            <form action="/moderation/resolve" method="POST">
                ...
                <button name="a" value="decline" class="nav-btn" style="background:#ef4444; border:none; cursor:pointer;">Decline / Dismiss</button>
            </form>
        </div>
        {% endfor %}
    </div></body></html>
    """
    return render_template_string(template, style=STYLE, reps=reps)
