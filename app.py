...
# Dynamic Maintenance Toggle via Render Env Var
...
class MyBot(discord.Client):
    ...
    async def setup_hook(self):
        ...
        print(f"✅ Bot synced. Maintenance is currently: {'ENABLED' if MAINTENANCE_MODE else 'DISABLED'}")
...
async def retire(interaction: discord.Interaction, name: str):
    ...
    await interaction.response.send_message(f"💀 **{name}** has retired.", ephemeral=False)

@bot.tree.command(name="check", description="Check player profile")
...
# --- WEB SERVER ---
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_KEY", "birdtiers_2026")
...
<head>
    ...
    <style>
        ...
        .profile-card { background: linear-gradient(145deg, #1a1d29, #14171f); border: 2px solid var(--accent); border-radius: 18px; padding: 25px; margin-bottom: 30px; display: flex; gap: 25px; align-items: center; box-shadow: 0 5px 15px rgba(0,0,0,0.3); }
        ...
        .player-row:hover { border-color: var(--accent); background: #1c1f2b; transform: translateX(5px); }
        ...
        .maint-msg { text-align: center; margin-top: 100px; color: var(--dim); }
        .maint-msg h1 { color: var(--accent); font-size: 40px; }
    </style>
</head>
<body>
    <div class="navbar">
    ...
    <div class="wrapper maint-msg">
        <h1>🛠️ UNDER MAINTENANCE</h1>
        <p>The leaderboard is currently offline for updates. Check back shortly!</p>
    </div>
    {% else %}
    <div class="mode-nav">
        ...
        <div class="profile-card">
            ...
            <div style="flex-grow:1;">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(90px, 1fr)); gap:8px; margin-top:10px;">
                    ...
                    <div style="background:rgba(0,0,0,0.3); padding:5px; border-radius:6px; font-size:11px; border:1px solid var(--border); text-align:center;">
                        {{m}}<br><b style="color:var(--accent)">{{t}}</b>
                    </div>
                    {% endfor %}
                </div>
                <a href="https://namemc.com/profile/{{spotlight.username}}" target="_blank" style="font-size:11px; color:var(--dim); margin-top:10px; display:inline-block;">View NameMC</a>
            </div>
        </div>
        {% endif %}
        {% for p in players %}
...
TOKEN = os.getenv("DISCORD_TOKEN")
MONGO_URI = os.getenv("MONGO_URI")
# Set this to True to lock the site for everyone except you (/login)
