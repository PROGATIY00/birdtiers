...
# Maintenance toggle (True = Locked for public)
...
async def rank(interaction: discord.Interaction, name: str, mode: app_commands.Choice[str], tier: str, region: str):
    ...
    await interaction.response.send_message(f"✅ Set {name} to {tier_up}", ephemeral=True)
    if LOG_CHANNEL_ID:
        channel = bot.get_channel(int(LOG_CHANNEL_ID))
        if channel: await channel.send(embed=embed)

@bot.tree.command(name="retire", description="Retire a player from a specific mode or globally")
...
async def retire(interaction: discord.Interaction, name: str, mode: app_commands.Choice[str]):
    ...
    await interaction.response.send_message(f"💀 Retired {name} ({mode.value}). Tier preserved but grayed out.", ephemeral=False)

# --- WEB UI ---
app = Flask(__name__)
app.secret_key = "birdtiers_ultimate_secret"

HTML_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <title>BIRDTIERS | Tier List</title>
    <style>
        ...
        .search-input { background: #0b0c10; border: 1px solid var(--border); padding: 8px 18px; border-radius: 20px; color: white; outline: none; width: 180px; transition: 0.3s; }
        ...
        .tier-box { background: rgba(0,0,0,0.3); padding: 8px; border-radius: 8px; font-size: 11px; border: 1px solid var(--border); text-align: center; }
        
        /* THE LEGACY LOOK: LT3 gray/tilded */
        .tier-retired { 
            opacity: 0.4; filter: grayscale(1); transform: rotate(-7deg); 
            border-style: dashed; background: transparent; 
        }
        .tier-retired b { color: var(--dim) !important; text-decoration: line-through; }
        ...
        .namemc-link { font-size: 11px; color: var(--dim); text-decoration: none; margin-top: 8px; display: block; text-align: center; }
        .namemc-link:hover { color: white; }
    </style>
</head>
<body>
    <div class="navbar">
    ...
    <div class="wrapper">
        ...
        <div class="profile-card">
            <div>
                ...
                <a class="namemc-link" href="https://namemc.com/profile/{{spotlight.username}}" target="_blank">NameMC</a>
            </div>
    ...
    for p in players_data:
        ...
        if mode_filter:
            ...
            if not is_retired:
                ...
                stats[u]["points"] += (p_val * 0.1) # Retired counts for less in Global
