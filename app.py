...
@bot.tree.command(name="rank", description="Update player rank")
...
async def rank(interaction: discord.Interaction, name: str, mode: app_commands.Choice[str], tier: str, region: str):
    ...
    tier_up = tier.upper()
    # We allow "RETIRED" as a tier here too, but the retire command is easier
    update_data = {"username": name, "gamemode": mode.value, "tier": tier_up, "region": region.upper(), "retired": False}
    
    players_col.update_one(
        {"username": {"$regex": f"^{name}$", "$options": "i"}, "gamemode": mode.value},
        {"$set": update_data}, upsert=True
    )
    await interaction.response.send_message(f"✅ **{name}** set to **{tier_up}** in **{mode.value}**", ephemeral=True)

@bot.tree.command(name="retire", description="Retire a player from a mode or globally")
@app_commands.choices(mode=[app_commands.Choice(name=m, value=m) for m in MODES] + [app_commands.Choice(name="Global (All Modes)", value="all")])
async def retire(interaction: discord.Interaction, name: str, mode: app_commands.Choice[str]):
    ...
    # We set 'retired': True but KEEP the 'tier' value so it shows LT3 (gray/tilted)
    players_col.update_many(query, {"$set": {"retired": True}})
    
    loc = "Globally" if mode.value == "all" else f"in {mode.value}"
    await interaction.response.send_message(f"💀 **{name}** has been retired {loc}. Their last ranks are preserved.", ephemeral=False)

# --- WEB UI ---
app = Flask(__name__)
app.secret_key = "birdtiers_legacy_v5"
...
<head>
    ...
    <style>
        ...
        .search-input { background: #0b0c10; border: 1px solid var(--border); padding: 8px 18px; border-radius: 20px; color: white; outline: none; width: 200px; }
        ...
        .tier-box { background: rgba(0,0,0,0.3); padding: 8px; border-radius: 8px; font-size: 12px; border: 1px solid var(--border); text-align: center; }
        
        /* THE LEGACY LOOK: Gray & Tilted */
        .tier-retired { 
            opacity: 0.4; 
            filter: grayscale(1); 
        ...
        .tier-retired b { color: var(--dim) !important; font-style: italic; }

        .player-row { background: var(--card); border: 1px solid var(--border); border-radius: 10px; padding: 10px 20px; margin-bottom: 8px; display: grid; grid-template-columns: 40px 45px 1fr 70px 110px; align-items: center; text-decoration: none; color: inherit; }
        ...
        <div class="profile-card">
            ...
            <div style="flex-grow:1;">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(95px, 1fr)); gap:10px; margin-top:10px;">
                    ...
                    <div class="tier-box {% if r.retired %}tier-retired{% endif %}">
                        {{r.gamemode}}<br><b style="color:var(--accent)">{{r.tier}}</b>
                    </div>
                    {% endfor %}
                </div>
            </div>
    ...
    for p in players_data:
        ...
        if mode_filter:
            ...
            if not is_retired: 
                ...
                stats[u]["points"] += TIER_DATA.get(p['tier'], 0)
            else:
                # Retired players still show their rank points but ranked lower
                stats[u]["points"] += TIER_DATA.get(p['tier'], 0) * 0.1 
