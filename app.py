...
partners_col = db_mongo['partners'] # New collection for partners
...
# Partner Commands
@bot.tree.command(name="partner", description="Manage MAGMATiers partners")
@app_commands.choices(action=[
    app_commands.Choice(name="Add", value="add"),
    app_commands.Choice(name="Remove", value="remove")
])
async def partner(interaction: discord.Interaction, action: str, name: str, image_url: str = None, link: str = "#"):
    if not interaction.user.guild_permissions.administrator:
        return await interaction.response.send_message("❌ Admin only.", ephemeral=True)
    
    if action == "add":
        partners_col.update_one({"name": name}, {"$set": {"name": name, "img": image_url, "link": link}}, upsert=True)
        await interaction.response.send_message(f"✅ Added partner: **{name}**", ephemeral=True)
    else:
        partners_col.delete_one({"name": name})
        await interaction.response.send_message(f"🗑️ Removed partner: **{name}**", ephemeral=True)
...
<head>
    ...
    <style>
        ...
        .insane-row { position: relative; background: var(--card) !important; padding: 12px 25px; border-radius: 12px; z-index: 1; }
        ...
        /* PARTNERS SECTION */
        .partners-section { margin-top: 50px; padding: 40px 0; border-top: 1px solid var(--border); background: #0f1117; text-align: center; }
        .partners-title { color: var(--dim); font-size: 14px; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 25px; }
        .partners-container { display: flex; justify-content: center; gap: 40px; flex-wrap: wrap; align-items: center; }
        .partner-img { height: 50px; filter: grayscale(1); opacity: 0.6; transition: 0.3s; }
        .partner-img:hover { filter: grayscale(0); opacity: 1; transform: scale(1.1); }

        .wrapper { max-width: 950px; margin: auto; padding: 25px; min-height: 70vh; }
...
<body>
    ...
    <div class="wrapper">
        ...
        <a href="/?search={{p.username}}" class="player-row {% if is_insane %}insane-row{% endif %}">
            ...
            <div style="font-weight:800; font-size:13px; color:var(--dim);">{{ p.region }}</div>
    ...
    <div class="partners-section">
        <div class="partners-title">Official Partners</div>
...
def index():
    ...
    for p in players_data:
        ...
        if mode_filter:
            if gm.lower() == mode_filter.lower():
                stats[u].update({"pts": val, "tier": tier, "retired": ret})
        else:
    ...
    return render_template_string(HTML_TEMPLATE, players=processed, all_modes=MODES, current_mode=mode_filter, search_query=search_q, partners=partners_data)
