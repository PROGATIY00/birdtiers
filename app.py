...
LOG_CHANNEL_ID = os.getenv("LOG_CHANNEL_ID")      # Staff Feed
STATUS_CHANNEL_ID = os.getenv("STATUS_CHANNEL_ID") # Public Network Status
...
async def maintenance(interaction: discord.Interaction, enabled: bool):
    ...
    if STATUS_CHANNEL_ID:
        try:
            ...
            desc = "Portal is currently down for technical updates." if enabled else "Maintenance complete. All systems are back online."
            
            embed = discord.Embed(title="📡 Network Status Update", description=desc, color=color)
            embed.add_field(name="Current State", value=f"**{status_text}**")
            await channel.send(embed=embed)
        except: pass

    await interaction.response.send_message(f"📡 Maintenance is now {'ON' if enabled else 'OFF'}", ephemeral=True)
...
<head>
    ...
    <style>
        ...
        .ts-header { background: #23272a; padding: 10px 15px; font-size: 13px; font-weight: bold; border-bottom: 1px solid #333; color: #888; }
...
<body>
    <div class="ts-modal">
        <div class="ts-header">MagmaTiers - Connection Interrupted</div>
        <div class="ts-body">
            <div style="color:#ff5555; font-weight:bold; font-size:14px;">SERVER MAINTENANCE</div>
            <div class="progress-bg"><div class="progress-bar"></div></div>
            <div class="info-box">> Status: 503_SERVICE_UNAVAILABLE<br>> System: Core Migration<br>> Reconnecting...</div>
        </div>
    </div>
</body>
</html>
...
<head>
    ...
    <style>
        ...
        .tier-badge { position: relative; cursor: help; background: #1c1f26; padding: 6px 12px; border-radius: 6px; font-weight: 800; font-size: 13px; color: var(--accent); text-align: center; }
        .peak-tooltip { visibility: hidden; background: #000; color: #fff; text-align: center; border-radius: 4px; padding: 4px 8px; position: absolute; z-index: 10; bottom: 125%; left: 50%; transform: translateX(-50%); font-size: 10px; border: 1px solid var(--accent); opacity: 0; transition: 0.2s; }
        ...
        .player-row { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 12px 25px; margin-bottom: 10px; display: grid; grid-template-columns: 45px 50px 1fr 70px 120px; align-items: center; text-decoration: none; color: inherit; }
    </style>
</head>
<body>
    <div class="navbar">
...
def index():
    ...
    for p in players_data:
        u, gm, tier, ret, pk = p['username'], p['gamemode'], p['tier'], p.get('retired', False), p.get('peak', 'LT5')
        val = TIER_DATA.get(tier, 0)
        if u not in stats: stats[u] = {"pts": 0, "region": p.get('region', 'NA'), "retired": True, "tier": "N/A", "peak": pk}
        if mode_filter:
            if gm.lower() == mode_filter.lower(): stats[u].update({"pts": val, "tier": tier, "retired": ret, "peak": pk})
        else:
    ...
    return render_template_string(HTML_TEMPLATE, players=processed, all_modes=MODES, current_mode=mode_filter, search_query=search_q)
