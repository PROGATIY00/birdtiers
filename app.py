...
def index():
    ...
    """
    return render_template_string(template, style=BASE_STYLE, nav=NAV_HTML, players=players, all_modes=MODES, mode_q=mode_q)

@app.route('/report', methods=['GET', 'POST'])
def report_page():
    if request.method == 'POST':
        db_manager.reports.insert_one({
            "player": request.form.get('player'), "reason": request.form.get('reason'),
            "evidence": request.form.get('evidence'), "status": "Pending", "time": datetime.datetime.utcnow()
        })
        return render_template_string("<html><head>{{ s|safe }}</head><body>{{ n|safe }}<div class='container'><h1>Report Sent!</h1><a href='/' class='btn'>Home</a></div></body></html>", s=BASE_STYLE, n=NAV_HTML)
    
    template = """
    <html><head>{{ style|safe }}</head><body>{{ nav|safe }}
    <div class="container" style="max-width: 600px;">
        <h1>Report a Player</h1>
        <div class="card"><form method="POST">
            <label>Username</label><input name="player" required>
            <br><br><label>Reason</label><textarea name="reason" rows="4" required></textarea>
            <br><br><label>Evidence Link</label><input name="evidence">
            <button type="submit" class="btn">Submit</button>
        </form></div>
    </div></body></html>
    """
    return render_template_string(template, style=BASE_STYLE, nav=NAV_HTML)
...
def moderation_page():
    ...
    template = """
    <html><head>{{ style|safe }}</head><body>{{ nav|safe }}
    <div class="container"><h1>Moderation Panel</h1>
        {% for r in reps %}
        <div class="card" style="margin-bottom:15px;">
            <h3>Player: {{ r.player }}</h3><p>{{ r.reason }}</p>
            <a href="{{ r.evidence }}" target="_blank" style="color:var(--accent);">Evidence</a>
            <form action="/moderation/resolve" method="POST" style="margin-top:15px;">
                ...
                <button name="a" value="dismiss" class="btn" style="background:#f87171;">Dismiss</button>
            </form>
        </div>
        {% endfor %}
    ...
    """
    return render_template_string(template, style=BASE_STYLE, nav=NAV_HTML, reps=reps)
